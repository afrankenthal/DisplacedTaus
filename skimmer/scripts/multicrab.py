...
def getOptions():
    ...
    parser.add_option('-s', '--sampleType',
                      dest = 'sampleType',
                      default = 'custom',
                      help = "Which kind of sample to process ('MC', 'signal', 'data', 'NoBPTX', 'cosmics', 'trig' or 'custom')",
                      metavar = 'STYP')
...
def main():
    ...
    skimmer_base_dir = os.path.join(os.environ['CMSSW_BASE'], 'src/DisplacedTaus/skimmer')
    ...
    if options.crabCmd == 'submit':
        ...
        for sample, dataset in total.items():
            ...
            config.General.requestName = 'ntuple_' + sample
